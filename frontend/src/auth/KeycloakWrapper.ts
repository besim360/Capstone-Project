 ...
 */
export default class KeycloakWrapper implements AuthWrapper {
  //#region Properties
  ...
  // The client detected at login
  private client = 'none specified';
  ...
  constructor() {
    if (!process.env.VUE_APP_KEYCLOAK_URL)
      throw 'KEYCLOAK_ENDPOINT is not defined';

    const [, , subdomain] = window.location.hostname.split('.').reverse();

    if (!subdomain || subdomain.length == 0)
      this.realm = process.env.VUE_APP_KEYCLOAK_REALM ?? 'app';
    else this.realm = subdomain;
    this.client = process.env.VUE_APP_KEYCLOAK_CLIENT_ID ?? `${this.realm}-client`
  ...
  SetupRouterGuards(router: vueRouter): void {
    router.beforeEach((to, from, next) => {
      ...
      if (allow_anonymous) {
        ...
        next();
      } else if (!allow_anonymous && this.IsAuthenticated()){
        next();
      } else if (!allow_anonymous && !this.IsAuthenticated()) {
